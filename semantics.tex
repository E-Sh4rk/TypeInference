...
\begin{document}
    ...
    \begin{lemma}[Substitution lemma]
      If $\Gamma, (x:t') \vdash e:t$ and $\Gamma \vdash e':t'$ with $t' \neq \Empty$ and $\dom \Gamma$ does not contain $x$, then $\Gamma \vdash e\{x\mapsto e'\}:t$.
    \end{lemma}
    ...
    We will suppose that all variables introduced by lambda abstractions are unique. When it is not the case, we can alpha-rename our terms.
    ...
    We will denote $\Gamma, (x:t')$ by $\Gamma'$.
    ...
    \begin{description}
      ...
      As $\dom \Gamma$ does not contain $x$, we know that $e$ does not contain $x$ neither, so the property becomes trivial.
      ...
      \item[If] TODO. May need a stronger version: no restriction on $\dom \Gamma$ but we apply a substitution to the env int he result.
    \end{description}
