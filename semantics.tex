...
\begin{document}
  ...
  \end{definition}
    ...
    For the case $\varphi.1$, we apply the outer induction hypothesis to get $\tyof {\Gamma'} {e\downarrow\varphi.0} \leq \tyof {\Gamma} {e\downarrow\varphi.0}$.
    We apply the induction hypothesis to get $\Gp p {\Gamma',e,t'} (\varphi) \leq \Gp p {\Gamma,e,t'} (\varphi)$. We can then conclude using the monotonicity of $\worra {} {}$.\\
    The case $\varphi.0$ follows from the previous case and the induction hypothesis.

    \qed

    \subsection{Substitution lemma}

    Let $\Gamma,e,t,x,t_x,e',t_{e'}$ as in the lemma statement. We will denote the substitution $\subst x {e'}$ by $\rho$.

    We proceed by induction on $e$. We suppose that, for any $e_s$ strict subexpression of $e$,
    \begin{align*}
      &\Gamma \vdash e_s:t_s \Rightarrow \Gamma \rho \vdash e_s \rho:t' \text{ with } t'\leq t_s\\
      &\Gamma \vdash e_s:t_s \Rightarrow \forall p \forall t' \forall \varpi.\ \Gp p {\Gamma\rho,e_s\rho,t'} (\varpi) \leq \Gp p {\Gamma,e_s,t'} (\varpi)
    \end{align*}
    ...
    \begin{description}
      ...
      \begin{itemize}
        ...
        \item Let $\Gamma_1 = \Gamma,\Gamma^+_{\Gamma,e_0,t_{\text{if}}}$. Let $\Gamma_1'=\Gamma_1,(e',\Gamma_1(x))$.
        ...
        By TODO, we have $\Gamma^+_{\Gamma\rho,e_0\rho,t_{\text{if}}} \leq \Gamma^+_{\Gamma,e_0,t_{\text{if}}}\rho$,
