...
\begin{document}
  ...
  \end{definition}
    ...
    \begin{theorem}[Subject reduction]
      Let $\Gamma$ an environment that has only non-reducible expressions (i.e. values, possibly with variables) in its domain, $e$ and $e'$ two expressions and $t$ a type.
    ...
    If we had $e\in\dom\Gamma$, then $e$ would not be reducible (it is an hypothesis we make on $\Gamma$).
    ...
    \begin{description}
      ...
      Thus, we know that $\Gamma \vdash v_1 : t_1$ with $t_1$ an arrow type, $\Gamma \vdash v_2 : t_2$, and $t_2 \in \dom {t_1}$.
      (should show that $t_2 \rightarrow t' \leq t_1$)

      $t_1 \leq t_2 \rightarrow \Any$ and $t_1 \leq t_2 \rightarrow t$
