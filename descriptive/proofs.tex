...
\begin{document}
  ...
  \maketitle
    ...
    \section{Theorems}
        ...
        We can also assume w.l.o.g. that every \Rule{If} rule is such that $\Gamma^p \leq \Gamma$. If it is not the case,
        we can easily transform the derivation by intersecting the $t'$ of every $\pvdash \Gamma p e t \varpi:t'$ in a \Rule{Path} rule with $\Gamma(\occ e \varpi)$
        (if $\occ e \varpi \in \dom \Gamma$)
        using the rules \Rule{PIntersect}, \Rule{PTypeof} and \Rule{Occ}. The rest of the derivation can easily be adapted by adding some \Rule{Subs} rules when needed.
        ...
        \begin{description}
          ...
          We get $\Gamma\rho\vdash e\rho:t_0$ and $\Gamma\rho\evdash + {e\rho} {t_{if}} \Gamma^+\rho$.

          Now we want to apply the induction hypothesis on $\Gamma^+\vdash e_1:t$ in order to have $\Gamma^+\rho\vdash e_1\rho:t$.
          However, we must show before that $\forall t'.\ \Gamma^+\vdash e_a:t'\Rightarrow \Gamma^+\rho\vdash e_b:t'$.

          TODO: move it in the Path case.

          Let $t'$ such that $\Gamma^+\vdash e_a:t'$.
          If $\Gamma^+ = \bot$, then $\Gamma^+\rho = \bot$ so we are done. So lets's suppose that $\Gamma^+ \neq \bot$.

          Let's separate the proof in two cases:
          \begin{itemize}
            \item If $e_a\not\in\dom{\Gamma^+}$. In this case, let's show that we have  $\Gamma\vdash e_a:t'$.
            Indeed, in the typing derivation of $\Gamma^+\vdash e_a:t'$, the \Rule{Occ} rule can only be applied on
            strict subexpressions of $e_a$. Among them, the only that can differ between $\Gamma$ and $\Gamma^+$ are
            the strict subexpression $e'$ of $e_a$ such that there exists $\varpi$ such that $\occ e \varpi \equiv e'$.

            Given the possible forms of $e_a$, the only such candidates are values. Moreover, as $\Gamma^+ \neq \bot$,
            $\forall v\in\dom{\Gamma^+}.\ \Gamma\vdash v : \Gamma^+(v)$. Thus we can derive $\Gamma\vdash e_a:t'$
            just by replacing every \Rule{Occ} rule applied to such values in the derivation of $\Gamma^+\vdash e_a:t'$
            by the relevant derivation.

            So we have $\Gamma\vdash e_a:t'$. Using the hypotheses of this lemma, we get $\Gamma\rho\vdash e_b:t'$.
            ...
            \item If $e_a\in\dom{\Gamma^+}$. Let's note $t_a=\Gamma^+(e_a)$. This time,
            we can't derive $\Gamma \vdash e_a:t'$ from $\Gamma^+\vdash e_a:t'$ because the rule \Rule{Occ}
            could be used on $e_a$ (which may not be a value).
            ...
            It gives us a derivation for $\Gamma^+ \vdash e_a : t''$ such that $t''\land t_a \leq t'$ and without any \Rule{Occ} applied to $e_a$.
            ...
            (from the hypotheses of the lemma). Still as before, we get a derivation for $\Gamma^+\rho\vdash e_b : t''$ by monotonicity.
            
            Now, we can append at the end of this derivation a rule \Rule{Intersect} with a rule \Rule{Occ} applied to $e_b$.
            As $(\Gamma^+\rho)(e_b) \leq \Gamma^+(e_a) = t_a$, we obtain by monotonicity a derivation for $\Gamma^+\rho\vdash e_b : t'$ (we can add a final \Rule{Subs} rule if needed).
          \end{itemize}

          We have just shown that $\forall t'.\ \Gamma^+\vdash e_a:t'\Rightarrow \Gamma^+\rho\vdash e_b\rho:t'$,
          thus we can apply the induction hypothesis on $\Gamma^+\vdash e_1:t$ and get $\Gamma^+\rho\vdash e_1\rho:t$.
          ...
          Now, let's show that $\forall t''.\ \Gamma^p \vdash e_a : t'' \Rightarrow \Gamma^p\rho \vdash e_b:t''$. TODO
            ...
            Thus, we can derive $(\Gamma,(\occ e \varpi:t_1))\rho\vdash e_b : t'$ by monotonicity.
