  ...
  TODO: add rules for pairs.
    ...
    \section{Theorems}
        ...
        \begin{corollary}[Value refinement 2]
          ...
          never uses the rule \Rule{Path} with $\varpi = \varpi'.x$ and such that $\forall y.\ \occ e {\varpi'.y}$ refers to a value.
        \end{corollary}
        ...
        \begin{description}
          \item[\Rule{PTypeof}] In this case we have $\Gamma \vdash \occ e {\varpi.x} : t'$. As $v \not\in \valsemantic{t'}$,
          we can derive $\Gamma \vdash \occ e {\varpi.x} : \Empty$ by using the rule \Rule{Intersect} and the rule \Rule{Abs} or \Rule{Const}.
          ...
          \item[\Rule{PEps+} and \Rule{PEps-}] This case is impossible.
          ...
          Now, by starting from $\pvdash \Gamma p e t \varpi:t_2$ and using the rules \Rule{PIntersect} and \Rule{PTypeof}, we can derive
        ...
        As $v$ is a value, the applications of \Rule{Path} can only have $\varpi=\epsilon$.
        Moreover, as $v \in \valsemantic{t} \Leftrightarrow p=+$:
        \begin{itemize}
          \item If $p=+$, any derivation $\pvdash \Gamma p v t \varpi:t'$ can only contains the rules \Rule{PTypeof}, \Rule{PIntersect}, \Rule{PSubs}
          and \Rule{PEps+} with $v \in \valsemantic{t}$. Thus we can easily derive $\Gamma \vdash v:t'$.
        ...
        any rule \Rule{Path} with $\varpi = \varpi'.x$ and such that $\forall y.\ \occ e {\varpi'.y}$ refers to a value.
