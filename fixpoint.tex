...
\begin{document}
    ...
    $\texttt{struct}(\varpi)[\vec s]$ with $\vec s$ a vector of elements of $S$ and $\varpi$ a path such that
    $\exists (e,t,p) \in \tenv.\ \occ e \varpi \equiv e'$.
    ...
    We need to show that $\Gp p {\Gamma'',e,t} (\varpi)$ has a DNF with only atoms of the form described above.    
    We proceed by induction on $\varpi$.

    Before treating the base case and the inductive case, we have to show that $\tyof {\occ e \varpi} {\Gamma'}$
    has a DNF with only atoms of the form... TODO.
    ...
    As there is also finitely many $\tsfun {\_}$ constructions in the successive $\Gamma(e)$ (trivial), there only can be finitely many different $\Gamma(e)$.
