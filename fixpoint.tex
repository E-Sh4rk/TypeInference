...
\begin{document}
    ...
    Let's prove that we can reach the greatest such fixpoint by applying successively the function $\Gaa p {\vec {i}}$ a finite number of times.\\
    ...
    We will show that, starting from $\Gamma$, successive applications of $\Gaa p {\vec {i}}$ stay in a finite set of environments.
    ...
    \begin{itemize}
      ...
      \item $\forall (e,t) \in \vec i\ \forall \varpi$, if $\occ e \varpi$ is a constant $c$, then $\basic{c} \in T$
    ...
    So we can deduce that there is finitely many possible environements that can be produced by the successive applications of $\Gaa p {\vec {i}}$.
    As $\Gaa p {\vec {i}}$ is reductive, we always reach a fixpoint in a finite number of iterations.
    By construction, this fixpoint is the greatest (as $\Gaa p {\vec {i}}$ is reductive and monotonic).
