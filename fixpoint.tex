...
\begin{document}
    ...
    In this proof we have assumed that there is no occurence of $\texttt{if}$ in the tests.
    If we want to prove the general case, we should proceed by induction on the number of $\texttt{if}$ in the tests.
    The proof above would become the base case. The inductive case would be quite similar:
    \begin{itemize}
      \item From the induction hypothesis, we can deduce that
      the set of all types that can be taken by the nested $\texttt{if}$ is finite.
      \item We must add those types in the set $T$.
    \end{itemize}

    TODO: Update once nested ite will be forbidden (or not)
