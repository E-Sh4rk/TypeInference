...
\begin{document}
    ...
    TODO: include Env \& Constr, find other names

    \begin{align*}
      &\Gamma^p_{\Gamma,\vec {i}}=\fixpoint_\Gamma (\Gamma^p_{\vec {i}})\\&\\
      &\Gamma^p_{\varnothing} (\Gamma) = \Gamma\\
      &\Gamma^p_{\cons {(e,t)} {\vec i}} (\Gamma) = \Gamma^p_{\vec i}(\Gamma,\Gamma^p_{\Gamma,e,t})\\&\\
    ...
    \[\valsemantic t = \{v \alt \vdash v : t\}\]

    TODO: Not really correct with our algorithmic type system... Define a separate typing system for values.

    TODO: Add a certain condition on the type of lambda abstractions in order to make the app case work.
    ...
    \begin{mathpar}
      ...
      \end{definition}
        ...
        TODO: Do not keep values in the env (otherwise, sr is not true).

        TODO: Lemma that states that, if we add an entry for a value in a well-formed environment, it will not chanhe the type of any non-value expressions.
